...
| [Glints](/docs/next/extractors/glints) | SEA job search (SG, ID, MY, VN) via GraphQL | WAF may block plain HTTP; Playwright/Camoufox fallback required in blocked environments | existing pipeline `searchTerms`, selected SEA country, `GLINTS_MAX_JOBS_PER_TERM` / automatic run budget | Posts `searchJobsV3` GraphQL, falls back to browser fetch on 403/WAF, emits location evidence, and de-duplicates by source id / URL |
...
- Use **Glints** when targeting Singapore, Indonesia, Malaysia, or Vietnam roles.
